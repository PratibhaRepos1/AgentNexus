# ChatBiz Marketing Site — Architecture

Promotion/marketing site for ChatBiz: home page, features, pricing, and free-demo booking.
...
ever heard of ChatBiz, so page-load speed and crawlability are the whole game. A client-rendered
...
- **`site` domain is a placeholder** (`https://chatbiz.app` in `astro.config.mjs` and
...
- **Demo form has no backend.** `src/pages/demo.astro` currently just shows a client-side "thanks"
  ...
  - the ChatBiz backend's leads/notifications pipeline (a new public `POST` endpoint), or
...
- **No testimonials/social proof yet** — the home page has a placeholder social-proof strip
  ...
  ChatBiz doesn't have paying customers yet. Replace once available.
